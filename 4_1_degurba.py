...
for idx, parent_log10 in enumerate(parent_impcrtlog10_columns):
    ...
    if not pd.isna(filtered_df[parent_log10].sum()):
        ...
        sns.boxplot(x=x, y=y, ax=axes[col], whis=[5, 95], color = color_palette[parent_log10])
        ...
        if pairs is not np.nan and p_values is not np.nan:
            annotator = Annotator(axes[col], pairs=pairs, data=filtered_df, x="degurba", y=parent_log10)
        ...
        axes[col].set_xticklabels([f'{item}\n n = {counts.get(item)}' for item in counts.index])
        axes[col].set_xlabel(f'ANOVA p-value: {pvalue_anova:.2e}')

        # y-axis in linear units
        y_ticks = axes[col].get_yticks()  # Get current y-tick positions 
        y_labels = [round(np.power(10, tick), 2) for tick in y_ticks]  # Convert and round to 2 decimals
        axes[col].set_yticks(y_ticks)  # Keep positions
        axes[col].set_yticklabels(y_labels)  # Replace labels with linear equivalents
        axes[col].set_ylabel("CRT-standardized, imputed, log10-transfrmed values")

        # title
        axes[col].set_title(parent_log10.replace('_impcrtlog10', ''), pad = 10)
...
for idx, substance in enumerate(pah_columns):
    ...
    if counts.values.sum() > 0:
        ...
        sns.boxplot(x=x, y=y, color=metabolites_colors[substance], ax=axes[row, col], whis=[5, 95])
        ...
        # Post-hoc test annotations
        annotator = Annotator(axes[row, col], pairs=pairs, data=filtered_df, x="degurba", y=substance+'_impcrtlog10')
        ...
        axes[row, col].set_xlabel(f'ANOVA p-value: {pvalue_anova:.2e}')
