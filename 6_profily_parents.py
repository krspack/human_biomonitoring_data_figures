...
selected = ['country', 'samplingmonth', 'heating_season', *parent_impcrt_columns]
...
def plot_profiles(dataframe, title, ylabel):
    ...
    for i, parent_compound in enumerate(parent_impcrt_columns):
        ...
        ax.bar(country_heating, values, bottom=bottom_values, label=parent_compound.replace('_impcrt', ''), color=impcrt_palette[parent_compound])
        bottom_values += values
...
selected = ['country', 'degurba', *parent_impcrt_columns]

df_copy['degurba'] = df_copy['degurba'].cat.rename_categories({'1': 'city', '2': 'town/suburb', '3': 'rural'})
...
def plot_profiles(dataframe, title, ylabel):
    ...
    country_degurba = sorted(dataframe.index)
...
for country_1 in countries:     
    ...
    data_city = df_hotelling[
        (df_hotelling['country'] == country_1) & (df_hotelling['degurba'] == 'city')
    ][columns_without_one].values
    data_town = df_hotelling[
        (df_hotelling['country'] == country_1) & (df_hotelling['degurba'] == 'town/suburb')
    ][columns_without_one].values
    data_rural = df_hotelling[
        (df_hotelling['country'] == country_1) & (df_hotelling['degurba'] == 'rural')
    ][columns_without_one].values
    ...
    all_results[country_1] = [{'p_value_city_town': p_value_city_town}, {'p_value_city_rural': p_value_city_rural}, {'p_value_town_rural': p_value_town_rural}]
...
# Hotelliguv test pro roky

# vybrat sloupce impcrt
df_hotelling = df_copy.copy()
df_hotelling = df_hotelling[~df_hotelling['country'].isin(['CZ', 'PL', 'CH'])]  # vyhodit se srovnani staty, ktere merily jen v jednom roce
df_hotelling = df_hotelling[['country', 'samplingyear', *parent_impcrt_columns]]
...
for country in countries:
    years = df_hotelling[df_hotelling.country == country]['samplingyear'].unique()
    countries_years[country] = list(years)

# test nebere dvojice, co maji cele shodne sloupce dat (chybejici hodnoty nahrazene medianem). Nahradit error hodnotou np.nan.
def safe_hotelling_t2(group_1, group_2):
    if (len(group_1) > 0 and len(group_2) > 0):
        try:
            return hotelling_t2(group_1, group_2)[2]
        except np.linalg.LinAlgError:
            return np.nan  # vrati np.nan jako p-value u problematickych dvojic
    return np.nan

# prochazeni statu
p_values = defaultdict(list)

for c, y in countries_years.items():  
    # Create dictionary of datasets for the current country
    data_country = {}
    for year in y:
        data_one_year = df_hotelling[
            (df_hotelling['country'] == c) & (df_hotelling['samplingyear'] == year)
        ][columns_without_one].values
        data_country[year] = data_one_year

    # Generate unique combinations of years for the country
    for d, dd in combinations(data_country.keys(), 2):
        p_value = safe_hotelling_t2(data_country[d], data_country[dd])
        couple = (d, dd)  # Tuple of year pairs
        p_values[c].append({couple: p_value})

print('Hotelling test results / p-values:')
print('==================================')
print('Differences between years per country (where data for comparison is available): ')
