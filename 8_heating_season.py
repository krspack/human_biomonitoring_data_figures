...
for idx, parent_compound_log10 in enumerate(parent_impcrtlog10_columns):
    ...
    # plot colors
    color_heating_season = make_grayer(parent_compound_log10.replace('_impcrtlog10', ''), parent_colors, 0.75)
    color_nonheating_season = parent_colors[parent_compound_log10.replace('_impcrtlog10', '')]
...
for idx, substance in enumerate(pah_columns):
    ...
    if 0 not in n_dict.values():
        ...
        axes[row, col].set_xticklabels(n_dict.items())
...
for idx, substance in enumerate(parent_columns):
    ...
    if grouped['count'].sum() > 0:
        ...
        # plot colors
        color_heating_season = make_grayer(substance, parent_colors, 0.75)
        color_nonheating_season = parent_colors[substance]
        palette = {True: color_heating_season, False: color_nonheating_season}
...
for idx, substance in enumerate(pah_columns):
    ...
    if grouped['count'].sum() > 0:
        ...
        ax.set_yticks(y_ticks)  # Keep positions
        ax.set_yticklabels(y_labels)  # Replace labels with linear equivalents
        ax.set_ylabel("log10-transformed")S
